...
declare type RepoOwnerType<T extends WebhookEvents> = WebhookEvent<T>["payload"] extends {
    repository: {
        owner: {
            login: string;
        };
...
declare type RepoNameType<T extends WebhookEvents> = WebhookEvent<T>["payload"] extends {
    repository: {
        name: string;
    };
} ? string : never;
/** Issue type (also pull request number), either number or never depending on the context */
declare type RepoIssueNumberType<T extends WebhookEvents> = WebhookEvent<T>["payload"] extends {
    issue: {
        number: number;
    };
...
declare type RepoResultType<E extends WebhookEvents> = {
